...
def fill_with_future_observation(arr):
    """backward-fill NaN values

    Args:
        arr (Numpy array): array of time series  

    Returns:
        Numpy array: original array with  NaN values filled with near future value
    """
    arr = arr[:, ::-1, :]
    out = fill_with_last_observation(arr)
    ...
    def __getitem__(self, idx):
        ...
        if self.masked_imputation_task:
            ...
            indices = np.random.choice(
                indices, round(len(indices) * self.artificial_missing_rate)
            )
            X_hat = np.copy(X)
